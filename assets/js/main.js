...
jQuery(document).ready(function ($) {
    ...
    $('.beds button').click(function () {
        $('.beds button').css('background-color', '#fff');
        $('.beds button').css('color', '#000');
        $(this).css('background-color', '#66c000');
        $(this).css('color', '#fff');
    });

    $('.bath button').click(function () {
        $('.bath button').css('background-color', '#fff');
        $('.bath button').css('color', '#000');
        $(this).css('background-color', '#66c000');
        $(this).css('color', '#fff');
    });

    $('.home-options button').click(function () {
        ...
        // $('.home-options button').toggle('background-color', '#fff');
        // $('.home-options button').toggle('color', '#000');
        // $(this).css('background-color', '#66c000');
        // $(this).css('color', '#fff');
    });

    if ($(window).width() < 768) {
        $('.s-map').removeClass('col-lg-7').addClass('col-lg-12');
    }

    $('.more-tools-ext-btn').click(function () {
        $('.dark-page').fadeOut();
    });

    $("#ex2").slider({});

    $('.more-tools-btn button').click(function () {
        $('.dark-page').fadeIn();
    });

    $('.dark-page').click(function (e) {
        var container = $(".more-tools");
    ...
    $('.top-filters button').click(function (e) {
    ...
    $('.fa-times').click(function () {
        $('.top-filters input').select();
        $(this).hide();
    });

    $('.top-filters input').click(function () {
        $('.fa-times').hide();
    });


    $('.register a,.mobile-register').click(function () {
        $('.popUp-register').fadeIn();
    });

    $('.popUp-register').click(function (e) {
        var container = $(".form");
    ...
    $('.filter-more-title').click(function () {
        if ($('.search-from span').hasClass('not-clicked')) {
            $this = $(this);
            $('.filter-more').fadeIn();
            $this.removeClass('not-clicked');
            $this.addClass('clicked');
            $this.text('فیلتر های کمتر');
            $('#angel').addClass('fa-angle-up').removeClass('fa-angle-down');
        } else {
            $('.filter-more').hide();
            $this.removeClass('clicked');
            $this.addClass('not-clicked');
            $this.text('فیلتر های بیشتر');
            $('#angel').addClass('fa-angle-down').removeClass('fa-angle-up');
        }

    });


    $('#home-f-slider').flickity({
        // options
        freeScroll: true,
        wrapAround: true,
        pageDots: false,
    ...
    $('.search-f-slider .carousel').flickity({
        freeScroll: true,
        wrapAround: true,
        pageDots: false,
        autoPlay: true,
    ...
    $('.fa-navicon').click(function () {
        $('.mobile-menu-shade').fadeIn();
        $('.mobile-menu').animate({right: '0px'}, 500);
    });


    $('.mobile-menu-shade').click(function (e) {
        var container = $(".mobile-menu");
        // if the target of the click isn't the container nor a descendant of the container
        if (!container.is(e.target) && container.has(e.target).length === 0) {
            $('.mobile-menu').animate({right: '-380px'}, 1000);
            $('.mobile-menu-shade').fadeOut();
        }
    ...
    $('.rent').on('click', function () {
        ...
        $(this).css('color', '#66c000');
        $('.sell').css('background-color', '#66c000');
        $('.sell').css('color', 'white');
    });
    $('.sell').on('click', function () {
        $(this).css('background-color', 'white');
        $(this).css('color', '#66c000');
        $('.rent').css('background-color', '#66c000');
